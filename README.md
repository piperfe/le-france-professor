...
- **WhatsApp Integration** — Students can chat with the tutor directly from WhatsApp. Text messages and voice notes are both supported. Incoming audio is downloaded from Meta, converted to WAV, and transcribed via whisper.cpp before entering the same tutor pipeline. One conversation per phone number; the first message always triggers a fresh greeting. `/vocabulary [mot]` explains a word in context; `/notebook` sends a formatted summary of the last 5 saved words plus a PDF export; `/notebook all` exports the full vocabulary list.
...
| [docs/decisions/](./docs/decisions/) | Architecture Decision Records — 29 decisions with source conversations |
