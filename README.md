...
- **Multi-conversation Sidebar**: A persistent sidebar lists all conversations by title (first 40 chars of the tutor's opening message). Clicking any entry navigates to that conversation. The "+" button creates a new one. The active conversation is highlighted with a rouge dot. The list is fetched server-side on every page load — no stale client cache.
