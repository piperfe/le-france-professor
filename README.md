...
- **WhatsApp Integration** — Students can chat with the tutor directly from WhatsApp. Incoming messages are received via Meta's webhook, processed by the same tutor pipeline, and the response is sent back via the Cloud API. One conversation per phone number; the first message always triggers a fresh greeting.
