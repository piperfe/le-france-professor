...
- **Slash Commands**: The input box supports `/vocabulary [word]` to get a contextual explanation of any French word. Typing `/` shows an autocomplete popup. The command is silent — no user bubble appears, only the vocabulary response rendered as a `📖 word` card inline in the chat. Each lookup is persisted to the vocabulary notebook.
- **Vocabulary Notebook**: A `📖 Vocabulaire` button in the chat header opens a slide-in drawer listing all words saved for the current conversation. Saved words are highlighted with an underline in the source tutor message — clicking a highlight opens the notebook with that entry remarked. The notebook is a bottom sheet on mobile and a side drawer on desktop. Word count persists across navigation.
- **Welcome Page — Recent Conversations**: The home page shows a "conversations récentes" list below the start button. Each entry shows the conversation title and a relative French date ("Hier à 14h32", "Il y a 3 jours", etc.) and navigates directly to that conversation. The list is fetched server-side on every load and is hidden when there are no prior conversations.
- **Multi-conversation Sidebar**: A persistent sidebar lists all conversations by title. Titles are generated by the LLM after the second student exchange (4–7 words summarising the topic). Before the title is generated, conversations show `Nouvelle conversation DD/MM HH:mm` so they are always distinguishable. Clicking any entry navigates to that conversation. The "+" button creates a new one. The active conversation is highlighted with a rouge dot. The list is fetched server-side on every page load — no stale client cache. On mobile the sidebar is hidden; a hamburger button in the chat header opens it as a full-height overlay drawer — tapping the backdrop or selecting a conversation closes it.
